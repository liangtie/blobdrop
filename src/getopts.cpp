...
bool parse(const QStringList &args) {
	...
	if (p.isSet(frontend_opt)) {
		...
		auto frontend_selection = std::views::zip(frontend_opts, std::views::iota(0u, frontend_opts.size() - 1)) | std::views::filter([&](const auto &i) { return std::string(std::get<0>(i)).starts_with(p.value(frontend_opt).toStdString()); });
